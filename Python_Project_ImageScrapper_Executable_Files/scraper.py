...
#Function to get only image URLS
#scroll_to_end - Just to Download more images
def fetch_image_urls(query: str, max_links_to_fetch: int, wd: webdriver, sleep_between_interactions: int = 1):
    ...
    while image_count < max_links_to_fetch:
        ...
        # Select/Search the image with the Image Tag holding the value
...
def search_and_download(search_term: str, driver_path: str, target_path='./images', number_images=10):
   ...
   # The above is trying to create a target Folder
#same code
    #target_folder = target_path+"/" +  ('_'.join(search_term.lower().split(' ')))

#Create a Target folder if does not exists
    if not os.path.exists(target_folder):
        os.makedirs(target_folder)
#Trying to open the Browser
    with webdriver.Chrome(executable_path=driver_path) as wd:
        res = fetch_image_urls(search_term, number_images, wd=wd, sleep_between_interactions=0.5)
# Calling function to Fetch the URL's
    counter = 0
    for elem in res:
